...
export const WordChainGame: React.FC<GameProps> = ({ onComplete, onExit, onRequestAd, highScore, lives, onLoseLife }) => {
    ...
    const handleWordTap = (word: string) => {
        ...
        if (board.correctWords.includes(word)) {
            ...
            if (newFound.length === board.correctWords.length) {
                setTimeout(() => {
                    playFanfare();
                    alert(`Categoria ${board.topic} Completa! (+1 Moeda)`);
        ...
        <div className="flex flex-col h-full bg-brand-bg">
            ...
            <div className="flex-grow p-4 flex flex-col">
                <div className="bg-blue-100 p-6 rounded-3xl text-center shadow-sm mb-6 border border-blue-200">
                    ...
                    <p className="text-blue-600 text-xs font-bold mt-2">Encontre {board?.correctWords.length} palavras (+1 <Coins size={10} className="inline"/>)</p>
                </div>
...
// ... RotationGame, ZenFocusGame, SumTargetGame, CardGame, PatternGame, EstimateGame, ColorMatchGame remain unchanged ...
export const RotationGame: React.FC<GameProps> = ({ onComplete, onExit, onRequestAd, highScore }) => {
    ...
    const handleGuess = (guessAngle: number) => {
        ...
        if(guessAngle === target) {
            ...
            setStreak(newStreak);
            setSessionScore(s => s + 2); // Reduced from 5 to 2
    ...
    const handleAdvantage = () => {
         onRequestAd(() => {
             const target = (baseAngle + rotationTask.value) % 360;
             const wrongs = options.filter(o => o !== target && o !== eliminatedOption);
             if (wrongs.length > 0) {
                 const toEliminate = wrongs[Math.floor(Math.random() * wrongs.length)];
                 setEliminatedOption(toEliminate);
                 alert("Uma opção errada foi eliminada!");
             }
         });
    }
                ...
                <div className="grid grid-cols-2 gap-6">
                    {options.map(a => (
                        <button 
                            ...
                            disabled={showCheckpoint || eliminatedOption === a}
                            className={`bg-white p-6 rounded-2xl shadow-sm flex items-center justify-center transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${eliminatedOption === a ? 'invisible' : ''}`}
                        >
...
export const SumTargetGame: React.FC<GameProps> = ({ onComplete, onExit, onRequestAd, highScore }) => {
     ...
     const resetRound = () => {
         ...
         // Increased difficulty: larger range based on level
         const variance = Math.floor(Math.random() * (10 + level * 5));
         ...
         // Larger max option for harder levels
         const maxOption = t - 1 + Math.floor(level / 2);
     ...
     const handleNextLevel = () => {
         setScore(s => s + 1); // Only 1 coin reward
         setLevel(l => l + 1);
         resetRound();
     }
                ...
                <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-50 p-6 animate-in fade-in" style={{pointerEvents: 'auto'}}>
                    <div className="bg-white rounded-3xl p-8 w-full max-w-sm text-center shadow-2xl">
                        ...
                        <div className="flex flex-col gap-3 mt-4">
                            <button onClick={handleNextLevel} className="w-full bg-brand-primary text-white py-3 rounded-xl font-bold text-lg flex items-center justify-center gap-2"><ArrowRight size={20}/> Próximo Nível (+1 pts)</button>
                            <button onClick={() => onComplete(score + 1)} className="w-full bg-green-100 text-green-800 py-3 rounded-xl font-bold text-lg flex items-center justify-center gap-2"><StopCircle size={20}/> Parar e Resgatar</button>
                        </div>
                    </div>
                </div>
            )}
    ...
    return (
        <div className="flex flex-col h-full bg-brand-bg">
            ...
            <div className="flex-grow flex flex-col items-center justify-center gap-8 p-6">
                <div className="h-8">{combo >= 2 && (<div className="bg-orange-100 text-orange-600 px-4 py-1 rounded-full font-bold text-sm animate-bounce flex items-center gap-1"><Flame size={14}/> COMBO x{combo} (+{Math.min(combo, 5)})</div>)}</div>
...
export const EstimateGame: React.FC<GameProps> = ({ onComplete, onExit, onRequestAd, highScore }) => {
    ...
    const startRound = () => {
        ...
        const opts = [c-2, c-1, c, c+1, c+2, c+5].sort(()=>Math.random()-0.5);
        setOptions(opts);
    ...
    const handleGuess = (guess: number) => {
        ...
        if (diff === 0) {
            playSuccessSound();
            setScore(s => s + 2); // Reduced from 10 to 2
            startRound();
        } else if (diff <= 1 && count > 15) { 
            playSuccessSound();
            setScore(s => s + 1); // Reduced from 5 to 1
            ...
            {phase === 'show' ? (
                ...
                <div className="flex-grow flex flex-col items-center justify-center p-6 gap-4">
                    ...
                    <div className="grid grid-cols-3 gap-4 w-full">
                        {options.map((opt, i) => (
                            <button key={i} onClick={() => handleGuess(opt)} className="bg-white p-4 rounded-xl shadow-sm border font-bold text-xl hover:bg-orange-50 active:scale-95">{opt}</button>
                        ))}
                    </div>
                    {hint && (
...
export const ColorMatchGame: React.FC<GameProps> = ({ onComplete, onExit, onRequestAd, highScore }) => {
    ...
    const handleAnswer = (ans: string) => {
        ...
        if(ans === correct) {
            playSuccessSound();
            setScore(s => s + 1); // Alterado para 1 moeda
            nextRound();
        } else {
            playFailureSound();
            alert("Errou!");
...
export const HiddenObjectGame: React.FC<GameProps> = ({ onComplete, onExit, onRequestAd, highScore }) => {
    ...
    const startRound = () => {
        ...
        const possibleColors = ['text-blue-500','text-green-500','text-yellow-500', 'text-red-500'];
        const tColor = possibleColors[Math.floor(Math.random() * possibleColors.length)];
        
        setTarget({icon: tIcon, color: tColor});
        
        // Gera todos os distratores
        const distractorIcons = icons.filter(i => i !== tIcon); // Ícones diferentes do alvo
        
        const newGrid = Array.from({length: 25}, (_, i) => {
            let dIcon = icons[Math.floor(Math.random()*icons.length)];
            let dColor = tColor;
            
            // Se o ícone sorteado for igual ao alvo, força a cor a ser diferente
            if (dIcon === tIcon) {
                const otherColors = possibleColors.filter(c => c !== tColor);
                dColor = otherColors[Math.floor(Math.random() * otherColors.length)];
            }
        ...
        // Coloca o alvo em uma posição aleatória
        const targetIdx = Math.floor(Math.random() * 25);
        newGrid[targetIdx] = { id: targetIdx, icon: tIcon, color: tColor, isTarget: true };
        
        setGrid(newGrid);
        
        // Decrease time as difficulty increases
        setTimeLeft(Math.max(10, 30 - Math.floor(score/2)));
    };

    const handleTap = (item: any) => {
        if(item.isTarget) {
            playSuccessSound();
            setScore(s => s + 1); // 1 moeda por acerto
            setTimeLeft(t => Math.min(30, t + 5)); // Bonus de tempo!
            startRound();
        } else {
            playFailureSound();
            alert("Item errado!");
    ...
    return (
        <div className="flex flex-col h-full bg-brand-bg">
            <GameHeader title="Oculto" icon={<Search size={24} className="text-gray-600"/>} onExit={onExit} currentCoins={score} onCollect={() => onComplete(score)} onGetAdvantage={handleAdvantage} advantageLabel="+15s (Vídeo)" highScore={highScore} scoreLabel="Máx Pontos" rightContent={<span className="font-bold text-red-500">{timeLeft}s</span>} />
...
export const MathRainGame: React.FC<GameProps> = ({ onComplete, onExit, onRequestAd, highScore }) => {
    ...
    const nextQ = () => {
        // Difficulty increase: higher numbers
        const range = 10 + (score * 2);
...
export const MovingHuntGame: React.FC<GameProps> = ({ onComplete, onExit, onRequestAd, highScore }) => {
    ...
    useEffect(() => {
        const speedFactor = 1 + (score * 0.1); // Increase speed by 10% each capture
        ...
        const loop = () => {
            setItems(prev => prev.map(p => {
                let nx = p.x + p.vx;
                let ny = p.y + p.vy;
                if(nx < 0 || nx > 85) p.vx *= -1;
                if(ny < 0 || ny > 85) p.vy *= -1;
                return {...p, x: nx, y: ny};
            }));
            reqRef.current = requestAnimationFrame(loop);
        };
    ...
    const handleTap = (isTarget: boolean) => {
        if(isTarget) {
            playSuccessSound();
            setScore(s => s + 1); // 1 moeda por captura
            setTimeLeft(t => Math.min(30, t + 3)); // Bonus de tempo
        } else {
            playFailureSound();
            onComplete(score);
        }
    ...
    return (
        <div className="flex flex-col h-full bg-brand-bg relative overflow-hidden">
            <GameHeader title="Caça" icon={<MousePointerClick size={24} className="text-red-500"/>} onExit={onExit} currentCoins={score} onCollect={() => onComplete(score)} onGetAdvantage={handleAdvantage} advantageLabel="+10s (Vídeo)" highScore={highScore} scoreLabel="Máx Pontos" rightContent={<span className="font-bold text-red-500">{timeLeft}s</span>} />
            <div className="flex-grow relative">
                {items.map(it => (
                    <button 
                        key={it.id} 
